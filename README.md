...
- **Ads show a mock ad popup in Play Mode**: a countdown (3s interstitial, 5s rewarded), then **Close Ad** (fires `afterAd`), or **Claim Reward** (fires `adViewed`) / **Skip** (fires `adDismissed`) for rewarded ads. This lets you verify pause-resume wiring and both reward outcomes by clicking. While the popup is up, input to the game behind it is blocked (uGUI clicks, legacy `Input` axis/button polling), matching how a real ad overlay behaves. Direct new Input System device polling (e.g. `Keyboard.current`) is not suppressed.
