...
- **Six more checks covering the project conditions a portal build tool reports.** Assets under a `Resources` folder and objects carrying a missing script reference are reported only, because the right action in both cases depends on what the project's own code expects. Read/Write enabled textures, Read/Write enabled meshes, mipmaps on sprites, and audio import settings are fixable. Each import fix re-reads the importer before writing, so a setting already corrected by hand since the last analysis is left alone.
